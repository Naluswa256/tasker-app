// coverage:ignore-file
// GENERATED CODE - DO NOT MODIFY BY HAND
// ignore_for_file: type=lint
// ignore_for_file: unused_element, deprecated_member_use, deprecated_member_use_from_same_package, use_function_type_syntax_for_parameters, unnecessary_const, avoid_init_to_null, invalid_override_different_default_values_named, prefer_expression_function_bodies, annotate_overrides, invalid_annotation_target, unnecessary_question_mark

part of 'user_model.dart';

// **************************************************************************
// FreezedGenerator
// **************************************************************************

T _$identity<T>(T value) => value;

final _privateConstructorUsedError = UnsupportedError(
    'It seems like you constructed your class using `MyClass._()`. This constructor is only meant to be used by freezed and you are not supposed to need it nor use it.\nPlease check the documentation here for more information: https://github.com/rrousselGit/freezed#custom-getters-and-methods');

UserModel _$UserModelFromJson(Map<String, dynamic> json) {
  return _UserModel.fromJson(json);
}

/// @nodoc
mixin _$UserModel {
  String get userDisplayName => throw _privateConstructorUsedError;
  String get userPersonalEmail => throw _privateConstructorUsedError;
  String? get userPhoneNumber => throw _privateConstructorUsedError;
  String? get userProfilePic => throw _privateConstructorUsedError;
  String get uid => throw _privateConstructorUsedError;

  Map<String, dynamic> toJson() => throw _privateConstructorUsedError;
  @JsonKey(ignore: true)
  $UserModelCopyWith<UserModel> get copyWith =>
      throw _privateConstructorUsedError;
}

/// @nodoc
abstract class $UserModelCopyWith<$Res> {
  factory $UserModelCopyWith(UserModel value, $Res Function(UserModel) then) =
      _$UserModelCopyWithImpl<$Res, UserModel>;
  @useResult
  $Res call(
      {String userDisplayName,
      String userPersonalEmail,
      String? userPhoneNumber,
      String? userProfilePic,
      String uid});
}

/// @nodoc
class _$UserModelCopyWithImpl<$Res, $Val extends UserModel>
    implements $UserModelCopyWith<$Res> {
  _$UserModelCopyWithImpl(this._value, this._then);

  // ignore: unused_field
  final $Val _value;
  // ignore: unused_field
  final $Res Function($Val) _then;

  @pragma('vm:prefer-inline')
  @override
  $Res call({
    Object? userDisplayName = null,
    Object? userPersonalEmail = null,
    Object? userPhoneNumber = freezed,
    Object? userProfilePic = freezed,
    Object? uid = null,
  }) {
    return _then(_value.copyWith(
      userDisplayName: null == userDisplayName
          ? _value.userDisplayName
          : userDisplayName // ignore: cast_nullable_to_non_nullable
              as String,
      userPersonalEmail: null == userPersonalEmail
          ? _value.userPersonalEmail
          : userPersonalEmail // ignore: cast_nullable_to_non_nullable
              as String,
      userPhoneNumber: freezed == userPhoneNumber
          ? _value.userPhoneNumber
          : userPhoneNumber // ignore: cast_nullable_to_non_nullable
              as String?,
      userProfilePic: freezed == userProfilePic
          ? _value.userProfilePic
          : userProfilePic // ignore: cast_nullable_to_non_nullable
              as String?,
      uid: null == uid
          ? _value.uid
          : uid // ignore: cast_nullable_to_non_nullable
              as String,
    ) as $Val);
  }
}

/// @nodoc
abstract class _$$_UserModelCopyWith<$Res> implements $UserModelCopyWith<$Res> {
  factory _$$_UserModelCopyWith(
          _$_UserModel value, $Res Function(_$_UserModel) then) =
      __$$_UserModelCopyWithImpl<$Res>;
  @override
  @useResult
  $Res call(
      {String userDisplayName,
      String userPersonalEmail,
      String? userPhoneNumber,
      String? userProfilePic,
      String uid});
}

/// @nodoc
class __$$_UserModelCopyWithImpl<$Res>
    extends _$UserModelCopyWithImpl<$Res, _$_UserModel>
    implements _$$_UserModelCopyWith<$Res> {
  __$$_UserModelCopyWithImpl(
      _$_UserModel _value, $Res Function(_$_UserModel) _then)
      : super(_value, _then);

  @pragma('vm:prefer-inline')
  @override
  $Res call({
    Object? userDisplayName = null,
    Object? userPersonalEmail = null,
    Object? userPhoneNumber = freezed,
    Object? userProfilePic = freezed,
    Object? uid = null,
  }) {
    return _then(_$_UserModel(
      userDisplayName: null == userDisplayName
          ? _value.userDisplayName
          : userDisplayName // ignore: cast_nullable_to_non_nullable
              as String,
      userPersonalEmail: null == userPersonalEmail
          ? _value.userPersonalEmail
          : userPersonalEmail // ignore: cast_nullable_to_non_nullable
              as String,
      userPhoneNumber: freezed == userPhoneNumber
          ? _value.userPhoneNumber
          : userPhoneNumber // ignore: cast_nullable_to_non_nullable
              as String?,
      userProfilePic: freezed == userProfilePic
          ? _value.userProfilePic
          : userProfilePic // ignore: cast_nullable_to_non_nullable
              as String?,
      uid: null == uid
          ? _value.uid
          : uid // ignore: cast_nullable_to_non_nullable
              as String,
    ));
  }
}

/// @nodoc
@JsonSerializable()
class _$_UserModel implements _UserModel {
  const _$_UserModel(
      {required this.userDisplayName,
      required this.userPersonalEmail,
      required this.userPhoneNumber,
      required this.userProfilePic,
      required this.uid});

  factory _$_UserModel.fromJson(Map<String, dynamic> json) =>
      _$$_UserModelFromJson(json);

  @override
  final String userDisplayName;
  @override
  final String userPersonalEmail;
  @override
  final String? userPhoneNumber;
  @override
  final String? userProfilePic;
  @override
  final String uid;

  @override
  String toString() {
    return 'UserModel(userDisplayName: $userDisplayName, userPersonalEmail: $userPersonalEmail, userPhoneNumber: $userPhoneNumber, userProfilePic: $userProfilePic, uid: $uid)';
  }

  @override
  bool operator ==(dynamic other) {
    return identical(this, other) ||
        (other.runtimeType == runtimeType &&
            other is _$_UserModel &&
            (identical(other.userDisplayName, userDisplayName) ||
                other.userDisplayName == userDisplayName) &&
            (identical(other.userPersonalEmail, userPersonalEmail) ||
                other.userPersonalEmail == userPersonalEmail) &&
            (identical(other.userPhoneNumber, userPhoneNumber) ||
                other.userPhoneNumber == userPhoneNumber) &&
            (identical(other.userProfilePic, userProfilePic) ||
                other.userProfilePic == userProfilePic) &&
            (identical(other.uid, uid) || other.uid == uid));
  }

  @JsonKey(ignore: true)
  @override
  int get hashCode => Object.hash(runtimeType, userDisplayName,
      userPersonalEmail, userPhoneNumber, userProfilePic, uid);

  @JsonKey(ignore: true)
  @override
  @pragma('vm:prefer-inline')
  _$$_UserModelCopyWith<_$_UserModel> get copyWith =>
      __$$_UserModelCopyWithImpl<_$_UserModel>(this, _$identity);

  @override
  Map<String, dynamic> toJson() {
    return _$$_UserModelToJson(
      this,
    );
  }
}

abstract class _UserModel implements UserModel {
  const factory _UserModel(
      {required final String userDisplayName,
      required final String userPersonalEmail,
      required final String? userPhoneNumber,
      required final String? userProfilePic,
      required final String uid}) = _$_UserModel;

  factory _UserModel.fromJson(Map<String, dynamic> json) =
      _$_UserModel.fromJson;

  @override
  String get userDisplayName;
  @override
  String get userPersonalEmail;
  @override
  String? get userPhoneNumber;
  @override
  String? get userProfilePic;
  @override
  String get uid;
  @override
  @JsonKey(ignore: true)
  _$$_UserModelCopyWith<_$_UserModel> get copyWith =>
      throw _privateConstructorUsedError;
}
